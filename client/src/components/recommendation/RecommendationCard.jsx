...
export const RecommendationCard = ({ video, layout = "grid" }) => {
  ...
  return (
    <div
      ...
      className={`flex rounded-xl border border-[#E2E8F0] bg-white shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-250 group overflow-hidden relative cursor-pointer ${isList ? "flex-col sm:flex-row gap-6 p-5" : "flex-col gap-3.5 p-5"
        }`}
    >
      {/* Watch Later Bookmark button */}
      <div className={`absolute top-7 right-7 z-20 transition-opacity duration-150 ${isWatchLater ? "opacity-100" : "opacity-0 group-hover:opacity-100"
        }`}>
        <button
          type="button"
          onClick={(e) => {
